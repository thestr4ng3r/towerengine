...
CDirectionalLightShadow::CDirectionalLightShadow(CDirectionalLight *light, int size)
{
	this->light = light;
	this->size = size;
	...
	tex_matrix = new float[16];
	memcpy(tex_matrix, CEngine::identity_matrix4, 16 * sizeof(float));
...
void CDirectionalLightShadow::Render(CWorld *world)
{
	...
	light_right = Cross(cam_dir, light_dir);
	...
	CVector *cam_frustum = world->GetCamera()->GetRelativeFrustumCorners(world->GetCamera()->GetNearClip(), 50.0);
	...
	CombineMatrix4(modelview, projection, tex_matrix);
